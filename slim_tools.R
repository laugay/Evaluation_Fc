...
Make.SNP.list <- function (file_in_1,file_in_2,file_fixed,populations,sample_size,N) {
  ...
  header_2 <- c("derived","ancestral")
  header_T <- c(header_1,header_2)
  ...
  for (pop in 1:populations) {
    ...
    if (num_of_mut > 0) {
      ...
      colnames(mut_table) <- header_1
      count <- matrix(0,num_of_mut,2)
      colnames(count) <- header_2
    ...
    if (length(mut_line) > 0 ) {
      ...
      if (num_of_mut > 0) {
        ...
        fmut_table <- matrix(NA,nrow=0,length(header_T))
        colnames(fmut_table) <- header_T
      }
    } else {
      fmut_table <- matrix(NA,nrow = 0,length(header_T))
      colnames(fmut_table) <- header_T
  ...
  colnames(mut_table_1) <- c("type","x","s","h","pop","time","n_pop.x","derived.x","ancestral.x")
  colnames(mut_table_2) <- colnames(fmut_table) <- c("type","x","s","h","pop","time","n_pop.y","derived.y","ancestral.y")
  ...
  SNP_list <- Reduce(function(...) merge(..., all = TRUE), L2)[-7]
  ...
  return( list(SNP_list=SNP_list,
               m2=m2_all,
               sampled_haplotypes_1_list=sampled_haplotypes_1_list,
